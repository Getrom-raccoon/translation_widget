...
def get_mask_card_number(card_number: str) -> str:
    ...
    if any(i.isalpha() for i in card_number):
        logger.warning("номер карты должен состоять только из цифр")
        return "номер карты должен состоять только из цифр"

    # Проверяем длину строки
    if len(card_number) != 16:
        logger.warning("Не соответствующая длина карты (16 цифр)")
...
def get_mask_account(account_number: str) -> str:
    ...
    if any(i.isalpha() for i in account_number):
        logger.warning("номер счета должен состоять только из цифр")
        return "номер счета должен состоять только из цифр"

    # Проверяем длину строки
    if len(account_number) != 20:
        logger.warning("Не соответствующая длина счета (20 цифр)")
